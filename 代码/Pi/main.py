...
if __name__ == '__main__':
    ...
    # 从起点出发
    logger.info("从起点出发")
    my_car.walk(0, -155, speed_mode.Fast)
    my_car.walk(300, 0, speed_mode.Fast)
    logger.info('-' * 50)

    logger.info("展开抓取臂和升降机构")
    my_car.my_store_arm.set(True)
    my_car.my_store_hand.set(False)
    logger.info("展开完成，前往二维码区")
    my_car.walk(350, 0, speed_mode.Fast)
    logger.info('-' * 50)

    my_car.walk(0, -20, speed_mode.Fast)
    my_car.my_store_lift.set(925)
    task_enum, task_str = task.scan_qr()
    my_car.my_arduino.monitor(task_str)
    my_car.my_store_lift.set(-925)
    my_car.walk(0, 20, speed_mode.Fast)
    logger.info(f"任务码枚举顺序：{task_enum}")

    # 从二维码区前往原料区
    logger.info("从二维码区前往原料区")
    my_car.walk(750, 0, speed_mode.Fast)
    logger.info('-' * 50)

    logger.info('第一次执行搬运任务')
    task.repeat_task(task_enum[:3], 925, CalculateTurnMode.INITIAL_TURN)

    # 实测发现从第二圈开始陀螺仪有误差累积，故引入一个偏差量
    # staging area to ingredient area
    logger.info("从暂存区前往原料区")
    my_car.walk(-885 - task.color_point[task_enum[2]], 0, speed_mode.Fast)
    my_car.turn(-90, speed_mode.Turn, CalculateTurnMode.COMPENSATED_TURN)
    my_car.walk(-400, 0, speed_mode.Fast)
    logger.info('-' * 50)

    logger.info('第二次执行搬运任务')
    task.repeat_task(task_enum[3:], 510, CalculateTurnMode.COMPENSATED_TURN)

    # 从暂存区返回起点
    logger.info("从暂存区返回起点")
    my_car.walk(-850 - task.color_point[task_enum[5]], 0, speed_mode.Fast)
    my_car.turn(-90, speed_mode.Turn, CalculateTurnMode.COMPENSATED_TURN)
    my_car.walk(-1825, 0, speed_mode.Fast)
    my_car.walk(0, 175, speed_mode.Fast)
    my_car.my_store_arm.set(False)
    my_car.my_store_hand.set(False)
    logger.info('-' * 50)
